...
(async function() {
    ...
    settings.highlightNewCome =
        value.highlightNewCome !== undefined
            ? Number(value.highlightNewCome)
            : settings.highlightNewCome;
    settings.highlightComeColor =
        value.highlightComeColor !== undefined
            ? Number(value.highlightComeColor)
            : settings.highlightComeColor;
    ...
    if (settings.isChTimetableWeekend) {
        ...
        if (sat >= 0) {
            t +=
                ...
                '>div:nth-child(' +
                (sat + 1) +
                ') article:not(.registeredProgs) .w7_mH{background-color:rgba(227,238,255,0.7);}';
            t +=
                tp +
                '>div:nth-child(' +
                (sat + 1) +
                ') article:not(.registeredProgs) .w7_mH:hover{background-color:rgba(222,233,250,0.7);}';
        }
        if (sun >= 0) {
            t +=
                tp +
                '>div:nth-child(' +
                (sun + 1) +
                ') article:not(.registeredProgs) .w7_mH{background-color:rgba(255,227,238,0.7);}';
            t +=
                tp +
                '>div:nth-child(' +
                (sun + 1) +
                ') article:not(.registeredProgs) .w7_mH:hover{background-color:rgba(250,222,233,0.7);}';
        }
    }
...
function PlaybuttonEditor() {
    if (!settings.isChTimetablePlaybutton) return;
    if (/^https:\/\/abema\.tv\/timetable\/dates\/.+$/.test(currentLocation))
            ...
            $('.playbutton').on('click', function(e) {
                ...
                if (
                    ...
                    setTimeout(
                        clickElement,
                        10,
                        $(EXTTsideR).find('a[href^="/now-on-air/"]')
                    ); //todo
                }
            });
        }
...
function putComment(commentText, userid, index, inmax, isSelf) {
    ...
    commentText = gcl.comefilter(commentText, userid, arFullNg, arUserNg, settings.isComeDel, settings.isUserDel, settings.isComeNg, settings.isDeleteStrangeCaps);
...
function openOption() {
    ...
    $('#commentBackColor')
        .val(settings.commentBackColor)
        .prev('span.prop')
        .text(
            settings.commentBackColor +
                ' (' +
                Math.round((settings.commentBackColor * 100) / 255) +
                '%)'
        );
    $('#commentBackTrans')
        .val(settings.commentBackTrans)
        .prev('span.prop')
        .text(
            settings.commentBackTrans +
                ' (' +
                Math.round((settings.commentBackTrans * 100) / 255) +
                '%)'
        );
    $('#commentTextColor')
        .val(settings.commentTextColor)
        .prev('span.prop')
        .text(
            settings.commentTextColor +
                ' (' +
                Math.round((settings.commentTextColor * 100) / 255) +
                '%)'
        );
    $('#commentTextTrans')
        .val(settings.commentTextTrans)
        .prev('span.prop')
        .text(
            settings.commentTextTrans +
                ' (' +
                Math.round((settings.commentTextTrans * 100) / 255) +
                '%)'
        );
    $('#isCommentPadZero').prop('checked', settings.isCommentPadZero);
    $('#isCommentTBorder').prop('checked', settings.isCommentTBorder);
    $('#itimePosition input[type="radio"][name="timePosition"]').val([
        settings.timePosition
    ]);
    $('#itimePosition').css(
        'display',
        settings.isTimeVisible ? 'flex' : 'none'
    );
    ...
    $('#iprotitlePosition input[type="radio"][name="protitlePosition"]').val([
    ...
    $('#iproSamePosition input[type="radio"][name="proSamePosition"]').val([
        settings.proSamePosition
    ]);
    ...
    $('#isChTimetablePlaybutton').prop(
        'checked',
        settings.isChTimetablePlaybutton
    );
    ...
    $('#isComeClickNGautoClose').prop(
        'checked',
        settings.isComeClickNGautoClose
    );
...
function setSaveClicked() {
    ...
    settings.changeMaxVolume = Math.min(
        100,
    ...
    settings.isCommentFormWithSide = $('#isCommentFormWithSide').prop(
        'checked'
    );
    ...
    settings.isComeClickNGautoClose = $('#isComeClickNGautoClose').prop(
        'checked'
    );
    settings.isShareNGword = $('#isShareNGword').prop('checked');
    settings.isDelOldTime = $('#isDelOldTime').prop('checked');
    settings.isMovieSpacingZeroTop = $('#isMovieSpacingZeroTop').prop(
        'checked'
    );
    settings.isMovieSpacingZeroLeft = $('#isMovieSpacingZeroLeft').prop(
        'checked'
    );
    settings.comeFontsize = Math.min(
        99,
    ...
    settings.audibleReloadWait = Math.max(
        0,
...
function setComeColorChanged() {
    ...
    for (var i = 0; i < jo.length; i++) {
        var jq = jo.eq(i);
        var jv = jq.val();
        jq.prevAll('span.prop').text(
            jv + ' (' + Math.round((jv * 100) / 255) + '%)'
        );
        p[i] = jv;
    }
...
function setOptionHead() {
    ...
    if (selHead && selFoot) {
        ...
        t +=
            selFoot +
            '>div>div:nth-child(3)>div:nth-child(1):hover{background:rgba(32,32,32,' +
            settings.panelOpacity / 255 +
            ')}';
        //フッターチャンネルアイコンの背景を透過
...
function applyCommentListNG(d) {
    ...
    for (let i = commentDivs.length - d; i < commentDivs.length; i++) {
        ...
        let filteredComment = gcl.comefilter(cinfo.message, cinfo.userid, arFullNg, arUserNg, settings.isComeDel, settings.isUserDel, settings.isComeNg, settings.isDeleteStrangeCaps);
        ...
        if (EXcomelist && isComeOpen()) {
            ...
            if (d > 0 && !scrolled) {
                ...
                if (
                    ...
                    isFootcomeClickable() &&
                    !hasNotTransformed($(EXside).add(EXchli))
                ) {
