...
export function isFootCommentClickable() {
    return (
        footerComment().length > 0 &&
        footerComment().css('pointer-events') !== 'none'
    );
}
...
export function findFooter() {
    ...
    if (!ch) {
        log.trace('findElement', '?footer(noch)');
        return $([]);
    }
    let chLogo = $('img[src*="/channels/logo/' + ch + '"]').rectFilter({
        top34d: true,
        right12l: true
    });
    ...
    let parents = chLogo
        .parents()
        .rectFilter({ notBodyParent: true, top34d: true });
    if (parents.length >= 1) {
        return parents.last();
    } else {
...
export function findFooterComment() {
    // コメントアイコンの先祖button
    let chLogo = footer().find(
        'img[src*="/channels/logo/' + getChannelByURL() + '"]'
    );
    if (chLogo.length === 0) {
        log.trace('findElement', '?footcome(!chLogo)');
        return $([]);
    }
    let commentIcon = $('[*|href*="/comment.svg"]:not([href])').containedBy(
        footer()
    );
    if (commentIcon.length === 0) {
        log.trace('findElement', '?footcome(!commentIcon)');
        return $([]);
    }
    ...
    for (let i = 0; i < parents.length; i++) {
        ...
        if (
            parents.eq(i).has(chLogo).length > 0 &&
            !parents.eq(i_1).is(footer())
        ) {
...
export function findViewCounter() {
    let p = $('p').filter(function() {
        return (
            $(this)
                .text()
                .indexOf('視聴数') >= 0 && $(this).siblings('span').length > 0
        ); // コメ欄と区別するためspanが兄弟かもみる
    });
    if (p.length === 0) {
        log.trace('findElement', '?vierCounter(!p)');
        return $([]);
    }
    let counter = p
        .parents()
        .rectFilter({ width12s: true, height14s: true, notBodyParent: true });
    if (counter.length === 1) {
        return counter.last();
    } else {
        log.trace('findElement', '?viewCounter(!counter)');
        return $([]);
    }
}
...
export function findSidemenu() {
    ...
    if (listIcon.length === 0) {
        log.trace('findElement', '?sidemenu(!listIcon)');
        return $([]);
    }
    let sidemenu = listIcon
        .parents()
        .rectFilter({ left34r: true, notBodyParent: true })
        .last();
...
export function findInfo() {
    return $('h3:contains(番組概要)')
        .parents()
        .rectFilter({ left12r: true, notBodyParent: true })
        .last();
}
...
export function findChannelList() {
    return $('a[href*="/timetable"]')
        .rectFilter({ top34d: true, left12r: true })
        .parents()
        .rectFilter({ left12r: true, notBodyParent: true });
}
