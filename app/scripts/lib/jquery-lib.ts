...
(function($) {
    ...
    $.fn.rectFilter = function(option) {
        ...
        return this.filter(function() {
            ...
            if (
                option.right34l &&
                b.left + b.width > (window.innerWidth * 3) / 4
            )
                return false;
            ...
            if (option.top34d && b.top < (window.innerHeight * 3) / 4)
                return false;
            ...
            if (
                option.bottom34d &&
                b.top + b.height < (window.innerHeight * 3) / 4
            )
                return false;
            if (option.width12s && b.width > window.innerWidth / 2)
                return false; // 幅が画面の半分より小さいか(大きいとfalse)
            if (option.width12b && b.width < window.innerWidth / 2)
                return false; // 〃大きいか
            if (option.width14s && b.width > window.innerWidth / 4)
                return false;
            if (option.width14b && b.width < window.innerWidth / 4)
                return false;
            if (option.width34s && b.width > (window.innerWidth * 3) / 4)
                return false;
            if (option.width34b && b.width < (window.innerWidth * 3) / 4)
                return false;
            if (option.height14s && b.height > window.innerHeight / 4)
                return false;
            if (option.height14b && b.height < window.innerHeight / 4)
                return false;
            if (option.height12s && b.height > window.innerHeight / 2)
                return false;
            if (option.height12b && b.height < window.innerHeight / 2)
                return false;
            ...
            if (
                ...
                return false;
            if (option.display && $(this).css('display') !== option.display)
                return false;
            if (
                option.displayNot &&
                $(this).css('display') === option.displayNot
            )
                return false;
