...
export function hasArray(array: ArrayLike<any>, item: any) {//配列arrayにitemが含まれているか
    return Array.from(array).includes(item);
}

export function getStorage(keys: string|Object|string[]|null, callback: (items: {[key: string]: any})=>void) {
    if (chrome.storage) {
        if (keys) {
...
export function setStorage(items: string|Object|string[], callback: ()=>void) {
    if (chrome.storage) {
...
//jQuery ajax
export function postJson(url: string, data: any, headers: any, callback: any, errorCallback: any) {
    if(isFirefox){
